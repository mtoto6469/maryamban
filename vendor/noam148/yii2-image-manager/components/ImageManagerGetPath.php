...
class ImageManagerGetPath extends Component { 
    ...
    /**
     * @var string The DB components name that the image model uses
     * This defaults to the default Yii DB components: Yii::$app->db
     * If this components is not set, the model will default to DB
     */
	...
	public function init() {
		...
		if (is_callable($this->databaseComponent)) {
		    // The database components is callable, run the user function
	...
	public function getImagePath($ImageManager_id, $width = 400, $height = 400, $thumbnailMode = "outbound") {
		...
		if ($mImageManager !== null) {
			...
			if (file_exists($sImageFilePath)) {
				...
				$return = null; //isset(\Yii::$app->controller->module->assetPublishedUrl) ? \Yii::$app->controller->module->assetPublishedUrl. "/upload/img_no-image.png" : null;
			}
		}
		return $return;
	}
	...
	private function _checkVariables() {
        ...
        if (! is_string($this->databaseComponent)) {
            throw new InvalidConfigException("Image Manager Component - Init: Database components '$this->databaseComponent' is not a string");
        }

        // Check to make sure that the $databaseComponent object exists
        if (Yii::$app->get($this->databaseComponent, false) === null) {
            throw new InvalidConfigException("Image Manager Component - Init: Database components '$this->databaseComponent' does not exists in application configuration");
        }
        ...
        if (($databaseComponentClassName = get_class(Yii::$app->get($this->databaseComponent))) !== ($connectionClassName = Connection::className())) {
            throw new InvalidConfigException("Image Manager Component - Init: Database components '$this->databaseComponent' is not of type '$connectionClassName' instead it is '$databaseComponentClassName'");
        }
    }
