var imageManagerModule = {
	...
	init: function(){
		//init cropper
		$('#module-imagemanager > .row .col-image-editor .image-cropper .image-wrapper upload#image-cropper').cropper({
		...
		$.ajax({
			...
			success: function (responseData, textStatus, jqXHR) {
				...
				//else set data
					...
					$("#module-imagemanager .image-info .thumbnail").html("<upload src='"+responseData.image+"' alt='"+responseData.fileName+"'/>");
					//remove hide class
	...
	editor: {
			...
			if(imageManagerModule.selectedImage !== null){
				...
				$.ajax({
					...
					success: function (responseData, textStatus, jqXHR) {
						...
						$('#module-imagemanager > .row .col-image-editor .image-cropper .image-wrapper upload#image-cropper').one('built.cropper', function () {
							//show cropper
		...
		applyCrop: function(pickAfterCrop){
			...
			if(imageManagerModule.selectedImage !== null){
				//set image in cropper
				var oCropData = $('#module-imagemanager > .row .col-image-editor .image-cropper .image-wrapper upload#image-cropper').cropper("getData");
				//call action by ajax
				$.ajax({
