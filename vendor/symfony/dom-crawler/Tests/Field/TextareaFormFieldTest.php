...
class TextareaFormFieldTest extends FormFieldTestCase
{
    public function testInitialize()
    {
        ...
        $node = $this->createNode('textarea', 'foo bar <h3>Baz</h3>');
        $field = new TextareaFormField($node);

        $this->assertEquals('foo bar <h3>Baz</h3>', $field->getValue(), '->initialize() sets the value of the field to the textarea node value');

        // Ensure that we don't do any DOM manipulation/validation by passing in
        // "invalid" HTML.
        $node = $this->createNode('textarea', 'foo bar <h3>Baz</h2>');
        $field = new TextareaFormField($node);

        $this->assertEquals('foo bar <h3>Baz</h2>', $field->getValue(), '->initialize() sets the value of the field to the textarea node value');
    }
}
